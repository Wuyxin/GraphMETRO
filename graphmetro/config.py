...
def set_cfg(cfg):
    ...
    # For GOOD benchmark
    cfg.dataset.shift_type = 'covariate'

    # Metric for evaluation
    cfg.dataset.metric = 'acc'
    ...
    cfg.dataset.split = [0.8, 0.1, 0.1] # [0.24, 0.06, 0.70]
    ...
    # cfg.train.search_temp = 0.5
    
    # Training (and validation) pipeline mode
    cfg.train.mode = 'standard'
    
    # MoE-specific param
    cfg.train.moe_hard = False
    
    # MoE-specific param
    cfg.train.moe_shared = False
    
    # Training (and validation) pipeline mode
    cfg.train.use_edge_attr = False
    ...
    # cfg.augment.type = 'none'

    # cfg.augment.prob = 0.5
    ...
    # # Whether enable search convs on different edge types
    # cfg.gnn.search = False
    
    # # Type of graph conv during search (if cfg.gnn.layer_type = 'mutable')
    # cfg.gnn.layer_search = ['sageconv','graphconv','leconv','transformerconv','gatconv']
    ...
    # Standard training learning rate
    cfg.optim.standard_lr = 0.001

    # Moe learning rate
    cfg.optim.moe_lr = 0.01
    
    # early stopping
    cfg.optim.patience = 100
    ...
    # scheduler: none, step, multistep, cos
    ...
    # Maximal number of epochs
    cfg.optim.moe_epoch = 100
    
    cfg.optim.warmup_epoch = 20
    # ----------------------------------------------------------------------- #
    # Shift options
    # ----------------------------------------------------------------------- #
    cfg.shift = CN()
    
    cfg.shift.test_types = ''
    
    cfg.shift.train_types = ''
    
    cfg.shift.p = [0.5]
    
    cfg.shift.k = [3]
    
    cfg.shift.fill_value = 'mean'
...
def load_cfg(cfg, args):
    ...
    cfg.wandb_exp_name = args.wandb_exp_name
...
def set_out_dir(cfg):
    ...
    Args:
        ...
        fname (string): Filename for the yaml format configuration file
    """
    if not cfg.wandb_exp_name is None:
        exp_name = f'{cfg.train.mode}-{cfg.wandb_exp_name}-seed={cfg.seed}'
    else:
        exp_name = f'{cfg.train.mode}-seed={cfg.seed}'
    cfg.out_dir = os.path.join(cfg.out_dir, 
                               cfg.dataset.name, 
                               exp_name
                               )
    os.makedirs(cfg.out_dir, exist_ok=True)
